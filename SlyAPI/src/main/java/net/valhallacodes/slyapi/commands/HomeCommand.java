...
public class HomeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            MessageUtils.sendMessage(player, SlyAPI.getInstance().getConfig().getString("messages.home-usage"));
            return true;
        }
        ...
        if (homeLocation.getWorld() == null) {
            MessageUtils.sendMessage(player, SlyAPI.getInstance().getConfig().getString("messages.home-world-not-found"));
            return true;
        }
        
        player.teleport(homeLocation);
        MessageUtils.sendMessage(player, SlyAPI.getInstance().getConfig().getString("messages.teleported-to-home")
                .replace("%home%", homeName));
